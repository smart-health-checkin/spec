...
[`conformance/`](conformance/) holds small single-capability test cases (a
request is valid, a transcript is built right, a DeviceResponse verifies…)
that every implementation runs in its own CI, each with a list of the cases
it fails today. See [`conformance/README.md`](conformance/README.md); they are
generated by `bun tools/conformance/generate.ts`.

Other repos use the fixtures and conformance cases by tag, not by copy. The
spec repo is tagged `vX.Y.Z` (currently `v1.0.0-draft.1`), and a consumer's
`scripts/fetch-spec.sh` clones that tag into a gitignored folder before its
tests run (`SPEC_REF` picks the tag; `SPEC_DIR=../spec` uses a local checkout).
Tags never move: when fixtures or cases change, tag a new version and bump
`SPEC_REF` in each consumer.
