...
// alignment plan (D1–D15) and the rewritten spec's requirements, not what any
...
type Expected = {
   ...
   *  "unsupported" (request side: SEL-8, SEL-9, SEL-10, FORM-1) or "unknown" (response side: D14). */
...
function add(c: Omit<Case, "id" | "requirements" | "status"> & { slug: string; status?: Case["status"]; pendingOn?: string }) {
  ...
  cases.push({ id: `${c.capability}/${slug}`, ...rest, requirements, status: status ?? "active", ...(pendingOn ? { pendingOn } : {}) });
}
...

{
  ...
  ok("form-versioned-canonical", "form.fhir by a versioned canonical (url|version).", request([formItem({ questionnaireCanonical: `${PHQ2}|1` })]), "CAN-1, CAN-4, D9");
  ...
  ok("unknown-kind", "An unknown content.kind: the request stands; that item is unsupported.", both(item({ id: "x", content: { kind: "example.extension", anything: 1 } })), "SEL-9, D5", { x: "unsupported" });
  ok("legacy-members-ignored", "Old selector members canonical and resource are ignored like any unknown member.", request([item({ content: { kind: "selection.fhir", profiles: ["http://hl7.org/fhir/us/core/StructureDefinition/us-core-patient"], canonical: "x", resource: "Patient" } })]), "JSON-3, D13");
...

{
  ...
  ok("declined-no-artifacts", "Everything declined, no Artifacts.", response([], [{ item: "patient", status: "declined" }]), "RSP-2, D10");
  ...
  // A bad status row affects only that item (D14).
  ok("unknown-status-code", "A status code outside the six: that item's outcome is unknown.", response([artifactFhir()], [{ item: "patient", status: "maybe" }]), "D14 (supersedes XV-3), RSP-3", { items: { patient: "unknown" } });
...

{
  ...
  pair("all-declined", "The patient declined everything: all declined, no Artifacts (D10).", req, response([], statusAll({ patient: "declined", insurance: "declined", intake: "declined" })), { valid: true }, "RSP-2, D10");
  ...
  pair("versioned-canonical-echo", "A versioned questionnaireCanonical is echoed exactly in QuestionnaireResponse.questionnaire.", request([formItem({ questionnaireCanonical: `${PHQ2}|1` })]), response([qr(`${PHQ2}|1`)], [fulfilled("intake")]), { valid: true }, "FORM-5, CAN-2, XV-10, D9");
  ...
  pair("missing-status-row", "An item has no status row: its outcome is unknown, the response stands.", req, response([artifactFhir(), cov, qr(PHQ2)], statusAll().slice(0, 2)), { valid: true, items: { intake: "unknown" } }, "D14 (supersedes XV-3), RSP-2");
  pair("duplicate-status-row", "An item has two status rows: its outcome is unknown, the response stands.", req, response([artifactFhir(), cov, qr(PHQ2)], [...statusAll(), fulfilled()]), { valid: true, items: { patient: "unknown" } }, "D14 (supersedes XV-3), RSP-2");
  pair("unknown-status-row", "A status row names an item the request doesn't have: it is ignored.", req, response([artifactFhir(), cov, qr(PHQ2)], [...statusAll(), fulfilled("nope")]), { valid: true }, "D14 (supersedes XV-3), RSP-2");
  ...
  pair("fhir-version-not-requested", "One Artifact uses a FHIR release the request didn't list: only that Artifact is unusable (D6).", req,
    ...
    { valid: true, artifacts: { a1: "rejected", a2: "accepted", a3: "accepted" } }, "XV-8 (SHOULD), D6");
  pair("fhir-version-bad-format", "One Artifact's fhirVersion is \"R4\", not a release version: only that Artifact is unusable (D6).", req,
    ...
    { valid: true, artifacts: { a1: "rejected", a2: "accepted", a3: "accepted" } }, "ART-2, XV-8 (SHOULD), D6");
}
...
type MdocKnobs = {
  ...
  x5chainBstr?: boolean;
  /** D15: encode the top-level map with the key "status" twice. */
...
/** CBOR map bytes with entries in the given order, duplicates allowed (for D15 cases). */
...

{
  ...
  ok("unknown-docrequest-key", "An unknown key in the DocRequest is ignored.", arg(deviceRequest(itemsRequest(), "1.0", true)), "ALG-2, D11");
  ok("unknown-itemsrequest-key", "An unknown key in the ItemsRequest is ignored.", arg(deviceRequest(itemsRequest({ extra: true }))), "ALG-2, D11");
  bad("wrong-protocol", "The request's protocol is org.iso.mdoc, not org-iso-mdoc.", arg(deviceRequest(), undefined, "org.iso.mdoc"), "protocol", "WRQ-2");
  {
    ...
    bad("padded-base64url", "deviceRequest is standard padded base64url, not unpadded.", arg(withId(k), undefined, "org-iso-mdoc", true), "base64url-padding", "WRQ-2");
  }
  ...
  bad("intent-not-bool", "intentToRetain is a string.", arg(deviceRequest(itemsRequest({ intent: "yes" }))), "intent-to-retain", "WRQ-5");
  bad("missing-request-info", "No requestInfo carrier (the request travels only there, D7).", arg(deviceRequest(itemsRequest({ requestInfo: null }))), "request-info-missing", "WRQ-5, D7");
  ...
  bad("unknown-version", "DeviceRequest version 9.0.", arg(deviceRequest(itemsRequest(), "9.0")), "device-request-version", "WRQ-3");
  {
    ...
    bad("encryption-key-wrong-curve", "encryptionInfo's recipient key has an unsupported crv.", arg(deviceRequest(), badKey), "encryption-key", "WRQ-7, ALG-2, D11");
    ...
    bad("encryption-info-wrong-tag", "encryptionInfo's first entry is not \"dcapi\".", arg(deviceRequest(), wrongTag), "encryption-info", "WRQ-7");
  }
  ok("companion-element-present", "The ItemsRequest also requests an old smart_request_b64u.* element: the request is read only from requestInfo, the extra element is ignored.",
    arg(deviceRequest(itemsRequest({ companion: true }))), "WRQ-6, D7");
  {
    ...
    bad("two-smart-doc-requests", "Two DocRequests for org.smarthealthit.checkin.1.", arg(withDocRequests(itemsRequest(), itemsRequest())), "doc-requests", "WRQ-4");
    // D15: a CBOR map with a duplicate key is invalid. ItemsRequest with docType twice.
    ...
    bad("duplicate-key-items-request", "The ItemsRequest map has the key docType twice.", arg(deviceRequest(dupItems)), "cbor-duplicate-key", "D15");
  }
}
...

{
  ...
  bad("wrong-origin", "Sealed with the transcript for another origin: must not open.", await seal(dr, synthOtherTranscript, synthPublicJwk), "hpke-open", "VRS-3, TR-5");
  {
    ...
    bad("wrong-response-tag", "The response's first entry is not \"dcapi\".", { protocol: "org-iso-mdoc", data: { response: base64UrlEncodeBytes(cborEncode(["other", decoded[1]])) } }, "dcapi-response", "VRS-2");
  }
  {
    ...
    bad("padded-base64url", "data.response is standard padded base64url, not unpadded.", { protocol: "org-iso-mdoc", data: { response: padded(sealed.data.response) } }, "base64url-padding", "VRS-2");
  }
  bad("wrong-protocol", "The credential's protocol is not org-iso-mdoc.", { protocol: "openid4vp", data: good.data }, "protocol", "VRS-2");
}
...

{
  ...
  });
  {
    ...
    add1("capture-v1-attached-equal", "The pre-fix capture: device signature payload attached but equal to the rebuilt DeviceAuthentication. Accepted.", readBytes(join(CAPTURE_V1_RESP, "device-response.cbor")), JSON.parse(read(join(CAPTURE_V1_RESP, "credential.json"))), { valid: true }, "VRS-7, D2", {
  ...
  await one("unknown-top-level-key", "An unknown top-level DeviceResponse key is ignored.", { extraTopLevelKey: true }, { valid: true }, "ALG-2, D11");
  await one("unknown-unprotected-header", "An unknown unprotected header label in issuerAuth is ignored.", { extraUnprotectedHeader: true }, { valid: true }, "ALG-2, D11");
  await one("attached-payload-other-session", "Device signature carries an attached payload built for another origin's transcript.", { attachDevicePayload: "other-session" }, { valid: false, reason: "device-signature" }, "VRS-7, D2");
  await one("tampered-device-signature", "One bit of the device signature flipped.", { tamperDeviceSignature: true }, { valid: false, reason: "device-signature" }, "VRS-7");
  await one("tampered-issuer-signature", "One bit of the issuer signature flipped.", { tamperIssuerSignature: true }, { valid: false, reason: "issuer-signature" }, "VRS-5, D1");
  await one("missing-validity-info", "The MSO has no validityInfo.", { omitValidityInfo: true }, { valid: false, reason: "mso-validity-info" }, "VRS-5, WRS-3, D2");
  await one("digest-mismatch", "The issuer-signed item changed after the MSO was signed.", { tamperItemAfterSigning: true }, { valid: false, reason: "digest" }, "VRS-6");
  await one("digest-missing", "The MSO has no digest for the item's digestID.", { omitDigest: true }, { valid: false, reason: "digest" }, "VRS-6");
  await one("document-doc-type", "The document's docType isn't org.smarthealthit.checkin.1.", { documentDocType: "org.iso.18013.5.1.mDL", msoDocType: "org.iso.18013.5.1.mDL" }, { valid: false, reason: "doc-type" }, "VRS-4");
  await one("mso-doc-type-mismatch", "The MSO's docType differs from the document's.", { msoDocType: "org.example.other" }, { valid: false, reason: "doc-type" }, "VRS-5");
  await one("status-not-ok", "DeviceResponse status is 10.", { status: 10 }, { valid: false, reason: "device-response-status" }, "VRS-4");
  await one("version-unknown", "DeviceResponse version is 9.0.", { version: "9.0" }, { valid: false, reason: "device-response-version" }, "VRS-4");
  await one("unknown-alg", "issuerAuth's protected alg is an unknown value.", { issuerAlg: -65535 }, { valid: false, reason: "alg" }, "ALG-2, D11");
  await one("unsupported-digest-algorithm", "The MSO's digestAlgorithm is SHA-512 (1.0 uses SHA-256).", { digestAlgorithm: "SHA-512" }, { valid: false, reason: "digest-algorithm" }, "ALG-1, ALG-2, D11");
  await one("missing-response-element", "The document carries another element instead of smart_health_checkin_response.", { elementIdentifier: "something_else" }, { valid: false, reason: "response-element" }, "VRS-8");
  ...
  await one("duplicate-key-device-response", "The DeviceResponse map has the key status twice.", { duplicateStatusKey: true }, { valid: false, reason: "cbor-duplicate-key" }, "D15");
  await one("two-documents", "Two documents in the DeviceResponse.", { secondDocument: true }, { valid: false, reason: "documents" }, "VRS-4");
  await one("expired", "Checked after the MSO's validUntil.", {}, { valid: false, reason: "mso-expired" }, "VRS-10 (SHOULD)", "pending", "VRS-10 is a SHOULD: decide whether conformance cases test SHOULDs.", "2026-10-05T00:00:00Z");
}
